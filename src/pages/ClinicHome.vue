...
const clinicFeatures = [
  { icon: 'fas fa-shield-alt', label: 'FHIR Compliant', desc: 'All records follow HL7 FHIR R4 interoperability standards' },
  { icon: 'fas fa-brain', label: 'AI-Assisted Notes', desc: 'SOAP notes generated in seconds with the AI Scribe Engine' },
  { icon: 'fas fa-lock', label: 'Privacy First', desc: 'Patient data isolated locally — no third-party cloud sharing' },
  { icon: 'fas fa-calendar-check', label: 'Online Booking', desc: 'Patients can self-schedule appointments 24/7' },
];
...
const openDays = computed(() => allHours.value.filter((r) => r.open).slice(0, 5));
const todayHours = computed(() => {
  const today = new Date().toLocaleDateString('en-US', { weekday: 'long' });
  return (clinic.value.hours || []).find((r) => r.day === today) || { open: false };
});
const todayStatus = computed(() => {
  const h = todayHours.value;
  if (!h.open) return 'Closed today';
  return `Today: ${h.from} – ${h.to}`;
});
        ...
        <div>
          <div class="cf-card" style="border-radius:1.25rem;padding:1.75rem;box-shadow:0 20px 50px rgba(0,0,0,.08)">
            <div style="display:flex;align-items:center;gap:.75rem;margin-bottom:1.25rem;padding-bottom:1rem;border-bottom:1px solid var(--border)">
              <div style="width:40px;height:40px;border-radius:.625rem;display:flex;align-items:center;justify-content:center" :style="`background:${clinic.brandColor || '#00D4B2'}22`"><i class="fas fa-clock" :style="`color:${clinic.brandColor || '#00D4B2'}`"></i></div>
              <div><p style="font-weight:700;font-size:.95rem;color:var(--text-strong);font-family:'Poppins',sans-serif">Today's Hours</p><p style="font-size:.75rem;color:var(--text)">{{ todayStatus }}</p></div>
              <div style="margin-left:auto"><span class="badge" :class="isOpenNow ? 'badge-brand' : 'badge-navy'">{{ isOpenNow ? 'Open Now' : 'Closed' }}</span></div>
            </div>
            <table class="hours-table">
              <tr v-for="row in openDays" :key="row.day" :style="row.isToday ? `color:${clinic.brandColor || '#00D4B2'}` : ''">
                <td style="font-weight:600;font-family:'Poppins',sans-serif;padding:.5rem .25rem" :style="row.isToday ? `color:${clinic.brandColor || '#00D4B2'}` : 'color:var(--text-strong)'">{{ row.day }}</td>
                <td style="text-align:right" :style="!row.open ? 'color:var(--text);opacity:.6' : ''">{{ row.open ? row.from + ' – ' + row.to : 'Closed' }}</td>
              </tr>
            </table>
            <div style="margin-top:1.25rem;padding-top:1rem;border-top:1px solid var(--border)" v-show="clinic.phone || clinic.whatsapp">
              <div v-show="clinic.phone" style="display:flex;align-items:center;gap:.625rem;margin-bottom:.5rem"><i class="fas fa-phone" :style="`color:${clinic.brandColor || '#00D4B2'}`" style="font-size:.8rem"></i><a :href="`tel:${clinic.phone}`" style="font-size:.85rem;font-weight:600;color:var(--text-strong)">{{ clinic.phone }}</a></div>
    ...
    <div class="container">
      <div class="two-col-grid" style="display:grid;grid-template-columns:1fr 1fr;gap:3rem;align-items:center">
        <div>
          ...
          <h2 class="section-title">{{ `About ${clinic.name || 'Our Clinic'}` }}</h2>
          <p class="section-sub" style="margin-bottom:1.5rem">{{ clinic.tagline || 'We are committed to providing the highest standard of patient care with compassion and clinical excellence.' }}</p>
          <div style="display:flex;flex-direction:column;gap:.75rem">
            <div style="display:flex;align-items:center;gap:.875rem">
              <div style="width:36px;height:36px;border-radius:.5rem;display:flex;align-items:center;justify-content:center;flex-shrink:0" :style="`background:${clinic.brandColor || '#00D4B2'}15`"><i class="fas fa-map-marker-alt" :style="`color:${clinic.brandColor || '#00D4B2'}`" style="font-size:.85rem"></i></div>
              <div><p style="font-size:.75rem;color:var(--text);font-weight:600">Address</p><p style="font-size:.88rem;font-weight:600;color:var(--text-strong)">{{ [clinic.address, clinic.city, clinic.state, clinic.pin].filter(Boolean).join(', ') || '—' }}</p></div>
            </div>
            <div style="display:flex;align-items:center;gap:.875rem" v-show="clinic.npi">
              <div style="width:36px;height:36px;border-radius:.5rem;display:flex;align-items:center;justify-content:center;flex-shrink:0" :style="`background:${clinic.brandColor || '#00D4B2'}15`"><i class="fas fa-id-badge" :style="`color:${clinic.brandColor || '#00D4B2'}`" style="font-size:.85rem"></i></div>
              <div><p style="font-size:.75rem;color:var(--text);font-weight:600">Registration / NPI</p><p style="font-size:.88rem;font-weight:600;color:var(--text-strong)">{{ clinic.npi }}</p></div>
            </div>
          </div>
          <button class="btn btn-brand" @click="apptModal = true" style="margin-top:2rem"><i class="fas fa-calendar-plus"></i>Schedule a Visit</button>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:.875rem" class="features-grid">
          <div v-for="f in clinicFeatures" :key="f.label" class="feature-card">
            <div style="width:40px;height:40px;border-radius:.625rem;display:flex;align-items:center;justify-content:center;margin-bottom:.875rem" :style="`background:${clinic.brandColor || '#00D4B2'}12`"><i :class="f.icon" :style="`color:${clinic.brandColor || '#00D4B2'};font-size:.9rem`"></i></div>
            <h4 style="font-weight:700;font-size:.9rem;color:var(--text-strong);margin-bottom:.3rem;font-family:'Poppins',sans-serif">{{ f.label }}</h4>
            <p style="font-size:.78rem;color:var(--text);line-height:1.5">{{ f.desc }}</p>
          </div>
        </div>
      </div>
    </div>
  ...
  <footer style="background:var(--color-secondary);padding:2.5rem 0;margin-bottom:1.5rem">
    <div class="container" style="display:flex;flex-direction:column;align-items:center;gap:1rem;text-align:center">
      ...
      <div style="display:flex;gap:1.5rem;margin-top:.25rem">
        <a href="#about" style="font-size:.78rem;color:rgba(255,255,255,.5)">About</a>
        <a href="#team" style="font-size:.78rem;color:rgba(255,255,255,.5)">Team</a>
        <a href="#services" style="font-size:.78rem;color:rgba(255,255,255,.5)">Contact</a>
      </div>
    </div>
  </footer>
  </div>
...
.feature-card { background:var(--bg-alt);border:1px solid var(--border);border-radius:1rem;padding:1.5rem;transition:all .3s; }
.feature-card:hover { transform:translateY(-4px);box-shadow:0 20px 40px rgba(0,212,178,.1);border-color:var(--brand); }
...
@media (max-width:480px) {
  .staff-grid,.services-grid,.features-grid { grid-template-columns:1fr!important }
}
</style>
