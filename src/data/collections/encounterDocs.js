...
// { id, encounterId, formId, recordId, attachedAt } — links a custom form's formData record to
// the encounter it was added during, same "documents attached to an encounter" pattern
// prescriptions/images above already use, just pointing at a generic formData record instead of
// a bespoke shape. Custom forms still go through the normal saveDataRecord()/formData path (see
// Designer.vue's Data Explorer), so this is purely an attachment/index layer on top — Designer's
// own view of a custom form's records is untouched by this. See
...
// Every custom-form record attached to one encounter, for one specific patient-journey form —
// Front Desk's "Additional Forms" step reads this to show just this visit's entries.
export function getEncounterCustomFormRecordIds(encounterId, formId) {
  return encounterCustomForms.toArray
    .filter((r) => r.encounterId === encounterId && r.formId === formId)
    .map((r) => r.recordId);
}
...
export function attachCustomFormRecord(encounterId, formId, recordId) {
  ...
  encounterCustomForms.insert({ id, encounterId, formId, recordId, attachedAt: new Date().toISOString() });
}

// Every custom-form record attached to one encounter, across every patient-journey form —
// Consultation Desk's Documents list reads this to fold custom-form entries in alongside
// images/prescriptions.
export function getEncounterCustomFormLinks(encounterId) {
  return encounterCustomForms.toArray.filter((r) => r.encounterId === encounterId);
}
